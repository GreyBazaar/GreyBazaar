...
export default class AcceptConfirm extends React.Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            ...
            deliveryDays: state.params.deliveryDays,
            email: state.params.email,
            
        })

        
            console.log('Summary of Buyer')
try{
             firestore().collection('BuyerRequests').doc(email).collection('MyRequests').doc(reqID)
            .onSnapshot(documentSnapshot => {
              console.log('User data: ', documentSnapshot.data());
              this.setState({
                  qualityType:documentSnapshot.data().qualityType,
